...
message StateValue {
  bool initial = 1;                // exactly one per enum
  bool terminal = 2;               // no outbound transitions
}
...
The generated/hand-written split is mechanical and enforced by CI. The contract is the
`// Code generated by ... DO NOT EDIT.` header plus an explicit list of generated roots
in `.gitattributes` marked `linguist-generated` — not a single filename suffix, since
stock plugins emit `*.pb.go` and `*_grpc.pb.go` and Ent emits `*_create.go`,
`*_query.go`, and friends. Within those roots, go-blocks' own plugins additionally use
`.gen.go` so a resource's generated file and its hook file sort together.
...
func (s *MenuItemService) PublishMenuItem(
	...
	case err != nil:
		return nil, errors.ErrorInternal("publish_menu_item", err)
	}

	// Generated adapter — delegates to the hand-written authz.Enforcer block.
	if err := policy.MenuItemPublish(ctx, s.authz, actor, row); err != nil {
		return nil, err
	}
	...
	if err := s.audit.Record(ctx, audit.Event{
		...
		Reason: req.GetReason(), Channel: "grpc", Outcome: audit.Success,
		RequestID: blocksauth.RequestID(ctx), At: s.clock.Now(),
	}); err != nil {
		return nil, err
	}
	return privacy.RedactMenuItem(ctx, s.pii, actor, toProto(out)), nil
}
```

```go
// policy_menu_item.gen.go — a thin adapter. It translates the proto `authorize`
// expression into an authz.Request; the decision itself belongs to the block.
func MenuItemPublish(
	ctx context.Context, e authz.Enforcer, a *blocksauth.Actor, r *ent.MenuItem,
) error {
	d, err := e.Check(ctx, authz.Request{
		Actor: a, Action: "menu_item.publish", Resource: "menu.MenuItem",
		Entity: r.ID,
		Attrs:  authz.Attrs{"outlet_id": r.OutletID, "status": r.Status},
		Tenant: a.TenantID,
	})
	if err != nil {
		return err
	...
	if !d.Allowed {
		return errors.ErrorForbidden("menu_item.publish", d.Reason)
	}
	return nil
}

// privacy_menu_item.gen.go — driven by field.pii. Also an adapter: the
// classification comes from the annotations, the masking decision from pii.
func RedactMenuItem(
	ctx context.Context, c pii.Classifier, a *blocksauth.Actor, m *menuv1.MenuItem,
) *menuv1.MenuItem {
	if !c.Permits(ctx, a, pii.PurposeContact) {
		m.OwnerEmail = pii.MaskEmail(m.OwnerEmail) // PII_CONTACT
	}
	return m
}
```
